...
# ---------- CONFIG ----------
API_TOKEN = "7953383377:AAEUxVx56xVpNkPE2pB5PC0VPxPzPjcGbYw"
...
async def edit_main_menu_callback(callback_or_user):
    if isinstance(callback_or_user, CallbackQuery):
        user_id = callback_or_user.from_user.id
    else:
        user_id = callback_or_user
    ...
    kb.adjust(1)
    if isinstance(callback_or_user, CallbackQuery):
        await clean_and_edit(callback_or_user, lang_data[lang]["main_menu"], kb.as_markup())
    else:
...
async def change_language_main(call: CallbackQuery):
    kb = InlineKeyboardBuilder()
    kb.button(text="🇺🇿 Оʻзбек (Кирил)", callback_data="lang_uz")
    kb.button(text="🇷🇺 Русский", callback_data="lang_ru")
    kb.button(text="🇬🇧 English", callback_data="lang_en")
    kb.adjust(1)
    await clean_and_edit(call, "🌍 Тилни танланг / Choose language:", kb.as_markup())
    await call.answer()
...
async def enter_text(message: Message, state: FSMContext):
    ...
    await state.clear()
    await edit_main_menu_callback(user_id)
